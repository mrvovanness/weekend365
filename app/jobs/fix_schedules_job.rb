...
class FixSchedulesJob
  ...
  def self.perform
    ...
    Survey.all.each do |survey|
      ...
      schedule = Resque.fetch_schedule("send_emails_#{survey.id}")
      # if a user has no schedule, add it to the array
      surveys_unscheduled << survey if schedule.nil?
    end

    if surveys_unscheduled.length > 0
      surveys_unscheduled.each do |survey|
        name = "send_emails_#{user.id}"
        config = {}
